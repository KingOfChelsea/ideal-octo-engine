  ...
  <div>
    <div class="card" >
      ...
      <div class="card-header">
        ...
        <span>日期：&nbsp;&nbsp;{{date}}<br /></span>
        <span>风力:&nbsp;&nbsp;{{ fengli}}<br /></span>
        <span>风向：{{fengxiang}}<br /></span>
        <span>星期：{{week}}<br /></span>
        <span>{{tip}}<br/></span>
      </div>
...
export default {
  ...
  methods: {
    getWeather(param) {
      this.axios
        .get(`https://api.vvhan.com/api/weather?city=${param}`)
        .then(
          (response) => {
            console.log("请求成功", response.data);
            this.temperature = response.data.info.high;
            this.date = response.data.info.date;
            this.position = response.data.city;
